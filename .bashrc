...
# enable zoxide to see hidden directories.
...
# zoxide + yazi
# gets directory path using zoxide fuzzy search and passes the best match to yazi for viewing files.
function zy() {
  local path
  path=$(zoxide query -- "$1")
  if [ -n "$path" ]; then
    flatpak run --command=yazi io.github.sxyazi.yazi "$path"
  else
    echo "Directory not found with zoxide: $1"
  fi
}
...
# zoxide + nvim
# gets directory path using zoxide fuzzy search and passes the best match to nvim to open as cwd.
function zn() {
  local path
  path=$(zoxide query -- "$1")
  if [ -n "$path" ]; then
